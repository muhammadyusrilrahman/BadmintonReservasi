<x-layouts.app :title="$title">
    ...
    <nav class="flex items-center gap-2 text-sm mb-6">
        ...
        <span class="text-slate-800 dark:text-white font-medium">Tambah Lapangan</span>
    </nav>
    ...
    <div class="max-w-2xl">
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
            ...
            <div class="px-6 py-5 border-b border-slate-200 dark:border-slate-800 bg-gradient-to-r from-[#1e3a5f]/5 to-transparent">
                <h1 class="text-lg font-bold text-slate-800 dark:text-white">Tambah Lapangan Baru</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Isi data lapangan badminton yang akan ditambahkan.</p>
            </div>

            {{-- Form --}}
            <form method="POST" action="{{ route('admin.courts.store') }}" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              ...
                              class="w-full px-4 py-2.5 text-sm bg-slate-50 dark:bg-slate-800 border
                                  ...
                                  rounded-xl text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 transition resize-none">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
